...
- bootstrap、lint 的 `lib/hermes_source_units` 是共享源码生成的内置副本，包括 schema/defaults。零第三方运行依赖，不增加服务或安装步骤。Provider 分发留在 P4。
...
P2 实现规范文本、结构切分、精确坐标、SourceUnit 持久化和读取；P3 接入 ledger/知识构建；P4 接入 Provider。配置目前可加载校验，但尚未驱动切片和模型。完整治理语义仍由治理工具和 lint 校验，骨架验证不替代它们。
