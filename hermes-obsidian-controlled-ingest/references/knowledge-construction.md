...
required retroactively for old artifacts. Source-only conversion may stop before this stage.
...
retain evidence marked `needs-qa`. No candidates is valid with an explicit `empty_reason`.

Before knowledge writes run:

```bash
python3 "<ingest-skill-root>/scripts/validate_knowledge_build.py" "<record>" --vault "<vault>" --phase plan
```

After writes run the same command with `--phase complete` before declaring the knowledge unit
complete. Record output paths in the existing ledger as usual. Validation is read-only; it checks
paths, fingerprints, ranges and decision consistency, not semantic truth, actual QA quality or
