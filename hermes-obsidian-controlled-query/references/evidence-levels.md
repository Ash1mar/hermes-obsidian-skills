...
| `source-backed` | Current non-failed converted Bundle/source evidence contains substantive answer content and resolves to an original PDF identity and page, but no durable card or concept has captured it yet. Non-failed warn/pending/qa_required/ambiguous/incomplete metadata may be retained as a qualification. | Can answer with original-PDF evidence; do not cite the conversion carrier. |
| `needs-qa` | A hard evidence-chain blocker, actual source conflict, or explicitly requested visual verification is incomplete. | Qualify the answer and state the concrete unresolved issue. |
...
- Non-failed `warn`, `pending`, `qa_required`, `ambiguous`, and `incomplete` metadata, including table/image labels, do not by themselves block a query answer or require another reference read. Use visible substantive content as `source-backed`, preserve the diagnostic in the trace, and do not promote it to a durable governed fact merely because the query used it.
...
- If a figure, page image, or table is part of the evidence, include its original PDF page, number or caption, containing section, and page region when reliably available. Keep converted asset paths in the trace. Use `needs-qa` for a hard evidence-chain blocker, actual source conflict, or explicit incomplete audit—not merely for non-failed diagnostic metadata or because visual review was skipped.
