...
`config/orchestration.json` keeps autonomous worker dispatch disabled until the canary rollout. Even when enabled, the Vault workflow must contain an explicitly armed, immutable allowlist of at most eight Pass slice IDs; every other node remains blocked. The Vault ledger remains authoritative; Kanban is rebuildable.
