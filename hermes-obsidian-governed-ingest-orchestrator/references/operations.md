...
worker-register-source is the standard source identity allocator. Runtime/contract errors block the card without writing a failed-source outcome. Domain governance commits and SourceUnit publication under a Kanban worker environment use the same workflow lock as cancellation. Successful cancellation therefore prevents subsequent authoritative source commits; in-flight conversion scratch output is not a committed artifact. These cooperative worker guards are not an OS security sandbox.
