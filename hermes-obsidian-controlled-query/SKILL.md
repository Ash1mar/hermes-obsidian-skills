...
Treat the compact candidates returned by `begin` as the complete operational input for the first inspection. They are packed within the fixed window to preserve strong-document diversity and add distinct matched question facets. Do not open the full candidate sidecar or trace state to look for a better list. A requested facet is an output attribute, value, condition, comparison, or action the user asked to receive; a noun, adjective, or phrase that merely limits the subject narrows answer scope and does not consume a candidate slot or create a claim. Select the smallest set that adds necessary requested outputs, and inspect all of those candidates in one call. Do not inspect contextual, comparative, applicability, or operational material unless the question requests it or it is necessary to resolve a concrete conflict or ambiguity. When a later exact registered section is genuinely needed, copy `document_path` verbatim from a returned compact candidate or evidence packet and append `::section-id`; never substitute a document ID, shortened bundle path, or guessed prefix. Do not probe `--help` or read script source to discover selector syntax. If that exact form fails, do not try alternate selector shapes; finalize from the inspected evidence, using `incomplete` when the remaining gap prevents a supported answer.
...
Use the minimum sufficient claim set. Every claim must answer a requested output attribute or action; subject qualifiers narrow scope but do not create claims. Merge closely related parameters when the same evidence supports them. Put a scope or evidence boundary in a short qualification on the affected claim instead of creating a separate background, comparison, applicability, or operational claim, unless the user explicitly asked for that information. Keep only unresolved items that materially change correctness or use of the answer. Write one short conclusion that synthesizes rather than repeats the claims.
...
Follow the `evidence_level_contract` returned by `inspect`. When it reports `ordinary_pass_quality: true` and `full_reference_required: false`, use its inline rules and do not read `references/evidence-levels.md`; a parameter, formula, table, or figure does not by itself justify that extra read. Read the full reference only when the contract reports a concrete QA/provenance trigger or when the inspected evidence reveals an actual conflict, answer-relevant ambiguity, or gap. Never promote `needs-qa` evidence into an authoritative reusable fact.
