...
`inspect` reads and registers evidence; it never performs visual verification and never grants verified status. Follow `finalize_contract.verification`. For the ordinary route, `required` is false, `verified_refs` is `[]`, and `visual_events` is `[]`; this requires `verified_evidence_refs: []` in the decision.

Also follow `finalize_contract.evidence`. Non-failed `warn`, `pending`, `qa_required`, `ambiguous`, and `incomplete` metadata, including table/image labels, appear under `diagnostics`; they remain directly usable at `usable_default_level` when substantive content and original source/pages are present. `read_reference` is false. Do not read `references/evidence-levels.md` for packet statuses. Exclude only `blocked_refs`; `blocked` records no substantive content, unresolved source/pages, content truncation, or failed/unavailable-class packet, source-map, or asset status. Actual source conflict and explicitly required incomplete visual verification remain hard blockers.

Before delivery, `inspect` applies one aggregate 18,000-character budget across the selected packets. It first removes exact duplicate asset Markdown and ordinary-route visual-audit fields, then—only when still over budget—keeps query-matched Markdown blocks and adjacent context. A packet or asset marked `delivery_excerpted` is a shortened agent copy; it is not a failed source read and must not be confused with `content_truncated`. The trace-side delivery metrics record packet reduction and separately measure the complete compact-JSON response, evidence packet, `finalize_contract`, and other response characters. They are stored in the diagnostic `evidence-packet-delivery` event rather than returned to the agent. Do not open the full trace or source to reverse this delivery optimization when the visible content already answers the question.
...
Follow `finalize_contract.events` as well. When `submit` is `[]`, submit `events: []`; query-session already records candidate selection, inspect, reading, assets, and provenance. Do not add a claim, comparison, or evidence ref merely to make an optional event reference count as used.
