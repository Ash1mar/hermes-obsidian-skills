...
共享库 0.4.0 只使用标准库。未知 schema 特性拒绝执行；不另造通用 JSON Schema 引擎。Ingest 与独立 Finalize Skill 在 `python -I -S` 隔离环境中完成准备、发布、回读、Build Finalize、Vault Finalize 和校验，不需要全仓库路径或额外安装。
...
8. 新 Provider 候选使用 source_units 扩展，不修改 coarse-recall/v1 的权威语义。P5 按 [ADR-0006](0006-release-driven-retrieval-projection.md) 直接投影 release 中可索引 SourceUnit，并同步更新生产和消费；除模型硬上限下已报告的 oversized 特例外不得重新切分。
...
P2.1 已按 ADR-0005 把 splitter 校正为可验证、可回退、可审计的共享 Chunk Engine。P3 已按 ADR-0004 实现 Pass/Reduce、稳定知识对象身份和 Build Finalize；P4 实现增量 Vault Finalize、导航与 release；P5 已实现 release 驱动的 Provider 投影和 Query 精确回读。各阶段当时的验收快照保存在本地 Git 忽略目录 `legacy docs/`；现行执行状态见[演进计划](../SOURCE_UNITS_EVOLUTION_PLAN.md)。
