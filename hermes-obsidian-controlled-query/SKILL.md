...
`query` creates the required trace, runs optional coarse recall and hierarchical routing in parallel, fuses the scope, automatically inspects the first three compact candidates, and returns only registered evidence packets plus a small dynamic `synthesis_contract`. It does not expose the candidate list. Its stdout is one compact JSON line; consume it directly and never pipe it through `head`, `tail`, or another line-limiting command. `delivery_metrics` reports packet compaction, while `output_metrics.stdout_chars` measures the complete serialized response including outer contract overhead. `output_metrics.output_complete: true` is the completion sentinel. Do not rerun the query, open the trace, or read source files to recover a downstream-truncated response. An unavailable or disabled Provider is non-blocking. `begin` and separate `inspect` remain compatibility/diagnostic commands and are not part of the normal model workflow.
...
Each returned packet has an ASCII `evidence_ref` such as `P1`. The trace stores the corresponding path, version, section, pages, original PDF, and viewer metadata. Cite only refs listed by `synthesis_contract.evidence_policy.usable_evidence_refs`; ignore `excluded_evidence_refs` unless their absence creates a material unresolved output. One excluded packet does not disqualify the usable packets. Retain only packet references when synthesizing claims; never copy provenance fields into finalization input. `inspect` is evidence reading and registration, not visual verification; do not describe it as `verify`, and never treat an inspected packet as a verified reference.
...
After synthesizing claims, run one atomic finalization call. Prefer stdin so shell quoting cannot alter the JSON and no temporary file is needed:

```bash
python3 "<query-skill-root>/scripts/query_session.py" finalize \
  <vault-root> <trace-id> --decision-stdin <<'JSON'
{"claims":[{"text":"Concise supported claim.","evidence_refs":["P1"]}],"conclusion":"Short conclusion."}
JSON
```

For `synthesis_contract.mode: ordinary-minimal`, first apply `semantic_sufficiency_gate`. When the visible usable packets answer every requested output, submit only `claims` and `conclusion`; the script defaults `status: completed`, `evidence_level: source-backed`, `verified_evidence_refs: []`, `events: []`, `unresolved: []`, and claim status. Packet validity alone does not establish answer completeness. When a requested output is unsupported, use the inline `incomplete_fallback` shape immediately: set `status: incomplete`, choose `source-backed` only when retaining supported requested claims and otherwise `gap`, and provide one material unresolved string. Do not read another reference merely to discover this fallback. Top-level decision and claim fields remain strict, packet refs must be inspected and usable, and only an actual visual check may add verification fields/events. The script assigns evidence/claim IDs, inherits provenance, and verifies the trace note exists. `--decision-json` remains supported for callers that can pass it safely. Do not create or patch a temporary manifest or decision file.
