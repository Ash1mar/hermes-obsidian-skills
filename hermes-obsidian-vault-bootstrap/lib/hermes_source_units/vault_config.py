"""Portable P1 bootstrap contract. No ingest or index readiness is implied."""
import json
import re
...
NOTICE = """\n## Source-unit rollout gate (P1)

This Vault uses the new source-unit architecture. Bootstrap initializes only
identity, storage directories and validated configuration. Source splitting,
unit reading, knowledge construction and Provider indexing are not connected yet.
Do not run legacy ingest/query against this Vault or claim it is query-ready.
Wait for the corresponding P2/P3/P4 implementations before consuming originals.
...
def validate_vault(vault):
    ...
    if manifest.get("source_units") != declaration(value):
        raise ValueError("Source-unit declaration/config fingerprint or P1 capability mismatch")
    ...
    return {"ok": True, "phase": "P1", "bootstrap_ready": True,
