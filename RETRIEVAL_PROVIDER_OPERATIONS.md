...
The intranet branch keeps both adapters disabled and explicitly configures
`http://qmd-like-rag:8781`. Its Compose example derives the Vault, Provider configuration and
host-bound state from `HERMES_HOST_DATA_ROOT=/data/data/hermes_agent0/.hermes`, joins the external
`hermes-runtime` network, and uses image `qmd-like-rag:0.5.0-intranet`. After container health is
ready, validate batch embedding dimensions and reranker responses, perform the first release sync,
compare `/status` with the Vault manifest, then enable Finalize and Query separately.
