"""Portable bootstrap contract with P3 source and knowledge-build capability."""
import json
import re
...
build and exactly read source units, then run P3 Pass/Reduce and Build Finalize.
Provider indexing is not connected yet. Do not run the legacy knowledge/query path
...
def declaration(config):
    ...
    return {"contract": "hermes-source-unit-vault/v1", "phase": "P3",
...
def validate_vault(vault):
    ...
    if manifest.get("source_units") != declaration(value):
        raise ValueError("Source-unit declaration/config fingerprint or P3 capability mismatch")
    ...
    return {"ok": True, "phase": "P3", "bootstrap_ready": True,
